    ...
    function showToast(message, type = 'success') {
      ...
      toast.className = `fixed top-5 right-5 px-5 py-4 rounded-lg text-white font-medium z-50 max-w-xs toast ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`
    ...
    function populateContactsTable() {
      ...
      contactsData.forEach((contact, index) => {
        ...
        row.className = `hover:bg-gray-100 cursor-pointer ${selectedContact === index ? 'bg-blue-100' : ''}`
        const fullName = `${contact.firstName} ${contact.lastName}`
        const isSent = sentEmails.some((sent) => sent.email === contact.email)
        row.innerHTML = `
                        <td class="px-3 py-3 text-left border-b border-gray-200">${fullName}</td>
                        <td class="px-3 py-3 text-left border-b border-gray-200">${contact.industry || 'N/A'}</td>
                        <td class="px-3 py-3 text-left border-b border-gray-200">${contact.email}</td>
                        <td class="px-3 py-3 text-left border-b border-gray-200">${isSent ? '<span class="bg-green-500 text-white px-2 py-1 rounded-full text-xs">Sent</span>' : ''}</td>
                    `
      })
    }
...
Web Dev Pro`
    ...
    // Tab Functions
		function switchTab(tabName, element) {
			...
			document.querySelectorAll('.tab').forEach((tab) => tab.classList.remove('active', 'border-b-indigo-500', 'text-indigo-500'))
			document.querySelectorAll('.tab-content').forEach((content) => content.classList.remove('active', 'block'))
			document.querySelectorAll('.tab-content').forEach((content) => content.classList.add('hidden'))

			// Add active class to selected tab and content
			element.classList.add('active', 'border-b-indigo-500', 'text-indigo-500')
			document.getElementById(`${tabName}-tab`).classList.remove('hidden')
			document.getElementById(`${tabName}-tab`).classList.add('active', 'block')
		}
    // Email Functions
